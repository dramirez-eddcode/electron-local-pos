...
export class BackupService {
   ...
   */
  async exportToUSB(usbPath: string, backupFilePath?: string): Promise<BackupResult> {
    try {
   ...
   */
  async detectUSBDevices(): Promise<Array<{ path: string; name: string; }>> {
    const devices = [];
   ...
   * Crear archivo ZIP
   */
  private async createZipFile(sourceDir: string, targetPath: string): Promise<void> {
   ...
   * Extraer archivo ZIP
   */
  private async extractZipFile(zipPath: string, targetDir: string): Promise<void> {
   ...
   */
  private async getSucursalInfo(): Promise<any> {
    try {
   ...
   */
  private async registerBackup(metadata: BackupMetadata, filePath: string): Promise<void> {
    try {
