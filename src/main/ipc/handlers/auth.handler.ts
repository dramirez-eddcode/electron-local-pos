...
import type { IpcResponse, Usuario } from '../../../shared/types/index.js';

export class AuthHandler {
  private readonly JWT_SECRET = process.env.JWT_SECRET || 'farmacia-ms-secret-key-2024';
  private readonly JWT_EXPIRES_IN = '8h';

  async login(event: any, credentials: { usuario: string; password: string }): Promise<IpcResponse<{ user: Usuario; token: string }>> {
    try {
  ...
  async logout(event: any, userId: number): Promise<IpcResponse<void>> {
    try {
      // Log de auditoría
  ...
  async verifyToken(event: any, token: string): Promise<IpcResponse<Usuario>> {
    try {
  ...
  async changePassword(event: any, data: { userId: number; currentPassword: string; newPassword: string }): Promise<IpcResponse<void>> {
    try {
