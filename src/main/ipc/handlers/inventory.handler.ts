...
export class InventoryHandler {
  ...
  async getProducts(event: any, params: {
    sucursalId: string;
    search?: string;
    limit?: number;
    offset?: number;
  ...
  async createProduct(event: any, productData: any): Promise<IpcResponse<any>> {
    try {
      const {
        CODIGO_PRODUCTO,
        NOMBRE_PRODUCTO,
      ...
      const existing = await this.db.query(
        'SELECT ID_PRODUCTO FROM PRODUCTO WHERE CODIGO_PRODUCTO = ? AND SUCURSAL_ID = ?',
        [CODIGO_PRODUCTO, SUCURSAL_ID]
      );
  ...
  async updateProduct(event: any, params: {
    ID_PRODUCTO: number;
    productData: any;
  }): Promise<IpcResponse<any>> {
    try {
      ...
      const existing = await this.db.query(
        'SELECT ID_PRODUCTO FROM PRODUCTO WHERE CODIGO_PRODUCTO = ? AND ID_PRODUCTO != ?',
  ...
  async deleteProduct(event: any, ID_PRODUCTO: number): Promise<IpcResponse<any>> {
    try {
      // Desactivar en lugar de eliminar (soft delete)
      await this.db.execute(
        'UPDATE PRODUCTO SET ACTIVO = 0, FECHA_MODIFICACION = datetime("now") WHERE ID_PRODUCTO = ?',
        [ID_PRODUCTO]
      );
  ...
  async getLaboratorios(event: any): Promise<IpcResponse<any[]>> {
    try {
      const laboratorios = await this.db.query(
        'SELECT * FROM LABORATORIO ORDER BY nombre_laboratorio ASC'
      );
  ...
  async getProductStats(event: any, sucursalId: string): Promise<IpcResponse<any>> {
    try {
  ...
  async exportToCSV(event: any, params: {
    sucursalId: string;
    search?: string;
    lowStock?: boolean;
  ...
  async importFromCSV(event: any, params: {
    csvData: string;
    sucursalId: string;
    userId: number;
  }): Promise<IpcResponse<any>> {
      ...
      for (let i = 1; i < lines.length; i++) {
        try {
          ...
          if (laboratorioNombre) {
            const labResult = await this.db.query(
              'SELECT id_laboratorio FROM LABORATORIO WHERE nombre_laboratorio = ?',
              [laboratorioNombre]
            );
  ...
  async adjustStock(event: any, params: {
    ID_PRODUCTO: number;
    ...
    try {
      ...
      // Actualizar stock
      await this.db.execute(
        'UPDATE PRODUCTO SET CANTIDAD_PRODUCTO = ?, FECHA_MODIFICACION = datetime("now") WHERE ID_PRODUCTO = ?',
