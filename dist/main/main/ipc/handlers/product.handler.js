...
export class ProductHandler {
    async search(event, searchParams) {
        try {
    ...
    async updateStock(event, data) {
        try {
            const { productId, quantity, operation } = data;
            // Verificar producto existe
            const producto = await db.query('SELECT CANTIDAD_PRODUCTO FROM PRODUCTO WHERE ID_PRODUCTO = ? AND ACTIVO = 1', [productId]);
            ...
            await db.execute('UPDATE PRODUCTO SET CANTIDAD_PRODUCTO = ? WHERE ID_PRODUCTO = ?', [newStock, productId]);
            return {
                success: true,
    ...
    async checkStock(event, productId, requiredQuantity) {
        try {
            const producto = await db.query('SELECT CANTIDAD_PRODUCTO FROM PRODUCTO WHERE ID_PRODUCTO = ? AND ACTIVO = 1', [productId]);
            if (producto.length === 0) {
                return {
