...
export class AdminHandler {
    ...
    async getUsers(event) {
      ...
      `);
            ...
            const usersWithPermisos = users.map((user) => ({
                ...user,
    ...
    async getUserTypes(event) {
      ...
      `);
            // Parsear permisos de JSON string a array
            const typesWithPermisos = types.map((type) => ({
                ...type,
    ...
    async createUser(event, userData) {
        try {
            ...
            const existingUsers = await db.query('SELECT ID_USUARIO FROM USUARIO WHERE LOGIN_USUARIO = ?', [LOGIN_USUARIO]);
     ...
     */
    async updateUser(event, userData) {
        try {
    ...
    async deleteUser(event, userId) {
        try {
            ...
            const users = await db.query('SELECT ID_USUARIO FROM USUARIO WHERE ID_USUARIO = ?', [userId]);
            if (users.length === 0) {
                return {
                    success: false,
            ...
            await db.execute('UPDATE USUARIO SET ACTIVO = 0 WHERE ID_USUARIO = ?', [userId]);
            return {
                success: true,
     ...
     */
    async createUserType(event, typeData) {
        try {
            ...
            const existingTypes = await db.query('SELECT ID_TIPOUSUARIO FROM TIPOUSUARIO WHERE NOMBRE_TIPO = ?', [NOMBRE_TIPO]);
     ...
     */
    async updateUserType(event, typeData) {
        try {
     ...
     */
    async deleteUserType(event, typeId) {
        try {
            if (!typeId) {
                return {
                    success: false,
            ...
            const usersWithType = await db.query('SELECT COUNT(*) as count FROM USUARIO WHERE ID_TIPOUSUARIO = ?', [typeId]);
            ...
            await db.execute('DELETE FROM TIPOUSUARIO WHERE ID_TIPOUSUARIO = ?', [typeId]);
            return {
                success: true,
