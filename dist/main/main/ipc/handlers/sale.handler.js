...
export class SaleHandler {
    async create(event, saleData) {
        try {
            ...
            for (const item of saleData.items) {
                const stockResult = await db.query('SELECT CANTIDAD_PRODUCTO FROM PRODUCTO WHERE ID_PRODUCTO = ?', [item.ID_PRODUCTO]);
    ...
    async procesarVenta(event, ventaData) {
        try {
            // Obtener datos del usuario actual desde el contexto o base de datos
            const userResult = await db.query('SELECT ID_USUARIO, SUCURSAL_ID FROM USUARIO WHERE NOMBRE_USUARIO = ? AND ACTIVO = 1 LIMIT 1', [ventaData.usuario]);
    ...
    async cancel(event, saleId, reason) {
        try {
    ...
    async getResumenVentas(event, params) {
        try {
